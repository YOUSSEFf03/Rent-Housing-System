...
const RentalCard = ({ property, navigation }) => {
    const { propertyName, location, features, pricing, images: propertyImages } = property;

    const imageSource = propertyImages[0]?.url
        ? { uri: propertyImages[0].url.startsWith('http') ? propertyImages[0].url : `file:///assets/${propertyImages[0].url}` }
        : require('../assets/bg-img10.jpg'); // Fallback image
        ...
        <View style={styles.card}>
            ...
            <View style={styles.cardContent}>
                ...
                <View style={styles.locationText}>
                    ...
                    <Text style={styles.locationTextContent}>{location.city}, {location.country}</Text>
                </View>
                <View style={styles.features}>
                    <View style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 8, }}>
                        ...
                        <Text style={styles.feature}>{features.bedrooms} bed</Text>
                    </View>
                    <View style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 8, }}>
                        <Svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <Text style={styles.feature}>{features.bathrooms} bath</Text>
                    </View>
                    <View style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 6, }}>
                        ...
                        <Text style={styles.feature}>
                            {features.surfaceArea.value} {features.surfaceArea.unit}
                        </Text>
                    </View>
                </View>
                <View style={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
                    <Text style={styles.price}>${pricing.rent}<Text style={styles.priceRate}>/{pricing.rateUnit}</Text></Text>
                    ...
                    <ButtonComponent
                        ...
                        color="secondary"
                        customeStyle={{ flex: 1 }}
                        onPress={() => navigation.navigate('RentalDetails', { property })}
                    />
                </View>
            </View>
        </View>
...
const styles = StyleSheet.create({
    ...
    locationTextContent: {
        fontSize: 14,
        color: '#6D6D6D',
        marginLeft: 4, // Add some spacing between the icon and text
    },
    features: {
        flexDirection: 'row',
